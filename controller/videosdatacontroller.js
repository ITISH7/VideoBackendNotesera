...
const createvideodata = async(req,res)=>{
    ...
    const new_topic = await topicdata.create({topicname:topicname,uri:topicuri});
   ...
   const find_college = await video.findOne({collegename:collegename,subjectname:subjectname});
   if(!find_college){
    const new_subject_data = await video.create({collegename:collegename,year:year,department:department,subjectname:subjectname});
    const new_unit_data= await coursedata.create({unitname:unitname,combinedname:combinedname});
    //console.log('new unit is ->',new_unit_data,'\n new unit obj id is ->',new_unit_data.id,'\n\n')
    const update_new_unit_data = await coursedata.updateOne({_id:new_unit_data.id},{$push:{topics:new_topic.id}})

    const update_new_subject = await video.updateOne({collegename:collegename,subjectname:subjectname},{$push:{coursedata:new_unit_data.id}})
    const get_subject_data = await video.find().populate({path:'coursedata',populate:{path:'topics',model:'topics'}})
    //console.log(get_subject_data)
    //res.json(get_subject_data);
}
else if(find_college){
    // agar subject ho to kya kare
    const is_course_available = await coursedata.findOne({combinedname:combinedname});
    //console.log('is_course available :-> ',is_course_available,'\n\n' );
    if(!is_course_available){
         const create_new_course = await coursedata.create({unitname:unitname,combinedname:combinedname,topics:new_topic.id});
         const upadte_existing_subject= await video.updateOne({subjectname:subjectname},{$push:{coursedata:create_new_course.id}});
    }
    else{
        // agar course ho to kya kare
        const update_existing_course = await coursedata.updateOne({combinedname:combinedname},{$push:{topics:new_topic.id}})
    }
}
const get_subject_data = await video.find().populate({path:'coursedata',populate:{path:'topics',model:'topics'}})
//console.log(get_subject_data)
...
const getyeardata= async(req,res)=>{
    ...
    const find_year = await video.find({collegename:collegename}).distinct("year")
    res.json(find_year);
}
const getdepartmentdata = async(req,res)=>{
    const collegename= req.params.collegename;
    const year =req.params.year
    const find_Department = await video.find({collegename:collegename,year:year}).distinct("department")
...
const getsubjectdata= async(req,res)=>{
    ...
    const find_subject=await video.find({collegename:collegename,year:year,department:department}).populate({path:'coursedata',populate:{path:'topics',model:'topics'}});
...
const get_coursedetails_from_a_subject = async(req,res)=>{
    ...
    const populated_data =  await find_subject.populate({path:'coursedata',populate:{path:'topics',model:'topics'}});
